...
- Your 3rd NC will be available **14 turns** after your 2nd NC
- Your 4th NC will be available **14 turns** after your 3rd NC
- Your 5th NC will be available **25 turns** after your 4th NC
- Your 6th NC will be available **25 turns** after your 5th NC
...
Since you roll for a time cop every turn (and multiple times when you have things like free-exit NCs), you can figure out a rough amount of predicted cops at each section of your run. For simplicity's sake, I will do this in a 1-day context and leave 2+ day as an exercise for the reader. Assuming that you are never taking any of the timeline repair options (and are singularly focused on juicing your Paradoxicity), you will need 4 + 7 + 14 + 14 + a few slush turns rolling for NCs = ** ~ 44 turns** to go from the 2% to 4% time cop rates. In those 39 turns, you would expect ~ 0.8 time cops (44 * 0.02), assuming no freebies in delay or freeruns. This leads to the following setup, in a 400 turn 1-day run, if you *never ever* take your Möbius ring off:

- You expect 0.8 time cops from turns 0 to 44 (at a 2% rate)
- You expect 6.9 time cops from turns 45 to 272 (at a 4% rate)
- You expect 9.3 time cops in turns 273 to the sorceress' tower at turn 388 (at a 8% rate)
...
A rough accounting of all the delay in a normal KOL run leads you to roughly 120 turns of delay, and about 50 turns where wanderers like time cops are as good as a native fight. If you were to (somehow) hold every single one of those turns for after you hit the 8% time cop rate, you'd be looking at >11 expected timecops, and could plop them all in delay. As is, I think you can hold off a good portion of those, but not -that- many. And some get eaten by the natural use of resources (for instance, losing anywhere from 8 to 18 with NC forcers + accountant fights in the Hidden City) or fights you actually need to do for alternate reasons (like quiet healers and burly sidekicks). My educated guess would be that most runs will need to run through 10-12 turns of delay in the first 44, 80% of the remaining delay in the middle, and the remainder over the final ~100 turn sprint. And if you only wear the ring in delay, the expected numbers from above turn into:

- You expect 0.2 time cops from turns 0 to 44 (at a 2% rate, with approximately 10 runnable turns)
- You expect 4.0 time cops from turns 45 to 272 (at a 4% rate, with approximately 100 runnable turns)
- You expect 3.2 time cops in turns 273 to the sorceress' tower at turn 388 (at a 8% rate, with approximately 40 runnable turns)

... for a total of 7.4 expected time cops per run. This roughly matches what speedsters are seeing; as an example, I got 9 in a recent 1-day run where I wore it outside of delay a decent bit, which led to 6 time cops in delay and 3 time cops out of delay. Which is all to say: this thing is incredibly complicated, almost to an annoying extent, but actual usage isn't super complicated. Leave it on for delay turns, take it off if a time cop doesn't help you (outside of the turns where you're rolling for your NCs). With mild management of your route, 6-8 cops in delay *over your whole run* is about what you can expect, with that amount being highly variable from run to run, and able to be tweaked ever-so-slightly upwards with careful management of delay to ensure a maximal amount is burned at the very end of your run. It also will be more powerful for slower runners; cops are markedly easier to encounter after turn 250, which can be \<100 turns at bleeding edge but almost 200 turns in the mid silver range.
